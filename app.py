...
class ChatBot():
    def __init__(self, name):
        self.name = name
        self.messages = [{"role": "system", "content": "Hello, I am Dave the AI. What can I do for you?"}]  # Store conversation history with initial message
    ...
    def process_user_request(self, user_input):
        if user_input is None:
            return "Please provide a valid user input."

        if any(i in user_input for i in ["incident", "tickets","ticket"]):
            ...
            if self.wake_up(user_input):
                ...
                self.messages.append(
                    {"role": "system", "content": user_input})  # Add user message to conversation history
                response = openai.ChatCompletion.create(
                    model="gpt-3.5-turbo",
                    messages=self.messages
                )
                self.messages.append({"role": "system", "content": response.choices[
                    0].message.content})  # Add AI response to conversation history
                return response.choices[0].message.content


ai = ChatBot(name="dev")
openai.api_key = "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"
